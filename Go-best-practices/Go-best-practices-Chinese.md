...
1. [配置](#配置)
1. [程序设计](#程序设计)
1. [Logging和instrumentation](#Logging和instrumentation)
1. [测试](#测试)
1. [依赖管理](#依赖管理)
1. [构建和部署](#构建和部署)
1. [结论](#结论)
...
TDD/BDD包带来了新的、不熟悉的DSL和控制结构，增加了你和你未来的维护者们在认知上的负担。我自己还没有看到哪个代码库上用了这个方法后，带来的好处能够抵消花费。就像全局状态一样，我相信这些包代表着一个错误的经济，而且通常是对其他语言和生态系统盲目崇拜行为的产物。在Go里，要像个真正写Go的人：我们已经有了一个能写出简单而富有表现力的测试的语言了——它叫做Go，并且你大概已经很了解它了。

也就是说，我确实认识到我自己的上下文和偏见了。就像我对于GOPATH的意见一样，我已经妥协很多了，就交给未来那些认为对于他们来说一个测试DSL或者框架比较合理的团队或组织吧。如果你知道你想用一个包，那就去做吧。只要确认你是为了一个说的通的理由就行。

另外一个非常有趣的话题已经为测试设计好了。Mitchell Hashimoto最近在柏林为一个话题作了一个演讲（[YouTube](https://www.youtube.com/watch?v=yszygk1cpEc)），我认为这是应该要读一下的。

总的来说，看起来最好的一件事就是，用一般函数化的方法写Go，这样依赖都被列举出来了，并且在必要时候提供小巧而有特定范围的interface。这样除了是为了做到一个好的软件工程师的纪律性本身之外，而且还不知不觉地优化了你的代码，使得它能容易测试。

> **Top Tip -- 用小巧的interface来为依赖建模**

就像上面刚提到的http.Client的例子一样，记住单元测试应该写成测试你正在测试的东西，其他的就不要测了。如果你在测试一个过程化函数，那就没有理由再去测试一个请求带进来的HTTP传输，或者将要写入的磁盘上的地址。提供输入输出用作interface参数的假的实现，把精力集中在方法或组件的业务逻辑上，其他不要管。
...
至于构建，藉由Dave Cheney的出彩的窍门，学到了一课：go install优于go build。install动词在$GOPATH/pkg中缓存了来自依赖的构建产物，这让构建更快。它也把二进制文件放在$GOPATH/bin中，这也让它更容易找到和调用。

> **Top Tip -- go install优于go build**

如果你生产二进制文件，别怕试试像gb这样的新的构建工具，它会显著地减少你的认知负担。相反地，记住自从Go1.5之后，交叉编译已经被內建了；仅仅需要设置合适的GOOS和GOARCH环境变量，并调用合适的go命令。因此再也不需要任何额外的工具了。

至于部署，比起Ruby或者Python，甚至JVM，我们Go程序猿有着相当简单的方法。有一个要记住：如果你在容器中部署，听从[Kelsey Hightower的建议](https://medium.com/@kelseyhightower/optimizing-docker-images-for-static-binaries-b5696e26eb07#.r4j4suwn2)并从FROM scratch开始。Go给了我们不可思议的机会；不使用它是可耻的。

更一般点的建议，在你选择一个平台或者编排系统的时候考虑清楚——即使你已经选了一个了。同样适用于跳到微服务阵营。一个优雅的庞然大物，作为一个AMI部署在自动伸缩的EC2组上，对于一个小团队来讲，设置起来是很费事的。抵抗、或者至少小心考虑炒作。

## 结论

头条建议：

1. 把$GOPATH/bin放到$PATH中，这样安装的二进制文件就能轻易地访问
1. 把库文件放在pkg/子目录下，把二进制文件放在cmd/子目录下
1. 一直使用完整import路径。不要使用相对路径
1. 参考Andrew Gerrand的命名规范
1. 只有main函数有权利决定哪些标记对用户可用
1. 使用struct连续的初始化方法，来避免无效的中间状态。必要时候使用行内stuct初始化
1. 使用默认的no-op实现，来避免nil检测
1. 让零值有用，特别是在config对象里
1. 让依赖显式化！
1. logger是依赖，就像其他组件、数据库句柄或者命令行标记等的引用一样
1. 用小巧的interface来为依赖建模
1. 测试你正在测试的东西
1. 用一个顶级的工具来为你的二进制文件将依赖管理在vender下
1. 库文件永远不要把依赖放进vendor。
1. go install优于go build

Go一直是一个比较保守的语言，并且它的成熟性是的它基本上没有带来什么意外并且实际上没有重大改变。因此，并且是可以预见到的是，社区也还没有大幅地转移它的关于什么是最好的实践的步调上。反而，我们已经看到了早些年被人所熟知的修辞或者箴言已经被具体化了，而且在上层已经稳步前进，比如设计模式、库以及程序结构已经被大家探索并转化到了Go中来了。

又将开始另外一个6年的快乐而充实的Go编程了。
